...
def audioGenerator(lst):
    ...
    for i in range(len(lst)):
        gtts.gTTS(text=lst[i],lang="hi",slow=True).save(f"AudioFiles/GeneratedFiles/{i}.mp3")
...
def stichAnnouncement(lst):
    ...
    final_audio = load_pre[0] + load_pre[1] + read_audio[0] + read_audio[1] + load_pre[2] + read_audio[2] + load_pre[3] + read_audio[3] + load_pre[4] + read_audio[4] +load_pre[5]
